...
static void llvm_unreachable(const char*) __attribute__ ((noreturn));
static void llvm_unreachable(const char* message)
{
	printf("%s\n", message);
	exit(1);
}
...
#define ALLOW_UIMM_DECODE(X) \
static DecodeStatus decodeUImmOperand_ ## X ## _(MCInst *Inst, uint64_t Imm, \
		int64_t Address, const void *Decoder) \
{ \
	return decodeUImmOperand(Inst, Imm, Address, Decoder, X);\
}
ALLOW_UIMM_DECODE(5);
ALLOW_UIMM_DECODE(6);
ALLOW_UIMM_DECODE(7);
ALLOW_UIMM_DECODE(8);
ALLOW_UIMM_DECODE(10);
ALLOW_UIMM_DECODE(16);
...
#define ALLOW_SIMM_DECODE(X) \
static DecodeStatus decodeSImmOperand_ ## X ## _(MCInst *Inst, uint64_t Imm, \
		int64_t Address, const void *Decoder) \
{ \
	return decodeSImmOperand(Inst, Imm, Address, Decoder, X);\
}
ALLOW_SIMM_DECODE(5);
ALLOW_SIMM_DECODE(16);
